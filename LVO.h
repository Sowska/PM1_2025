...
resultLocLVO localizarLVO(LVO lista, int dni, float *costo)
{
    resultLocLVO loc;
    loc.exito = 0;
...
int bajaLVO(Prestador prestador, LVO *lista, float *costo, int flag)
{ // el flag activado indica baja con confirmacion

    (*costo) = 0.0; // Inicializo el costo a 0
    float costoloc = 0;
    int opcion = 0;
...
void modificarLVO(int dni, LVO *lista)
{
    ...
    int opmod = 0;
    resultLocLVO resultado = localizarLVO(*lista, dni, &costoDespreciado);
    if (resultado.exito)
    {
        printf("El prestador encontrado es:\n");
...
Prestador evocarLVO(LVO lista, int dni, float *costo)
{
    (*costo) = 0; // Inicializo el costo a 0
    Prestador prestador;
